...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "../../ui/select";
  ...
  export default function StudentForm({handleSubmit,values}) {
    ...
    const onSubmit = async values => {
        ...
        ({status, data}) => {
          if (status === 200) {
            ...
            reset()
            navigate(ADMIN_MANAGE_STUDENTS_ROUTE)
          }
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  {Parent.map((parent, key) =>
                    <SelectItem key={key} value={parent.id.toString()}>{parent.nom} {parent.prenom}</SelectItem>)
                  } 
                </SelectContent>  
              </Select>
              <FormMessage/>
